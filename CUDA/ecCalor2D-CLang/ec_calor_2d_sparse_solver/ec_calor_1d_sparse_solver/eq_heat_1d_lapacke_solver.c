...
int main(int argc, char const *argv[])
{
    ...
    A       = allocate_memory_matrix_flat_double(rows_number, columns_number);
    ...
    get_1D_heat_equation_matrix_dense(A, rows_number, columns_number, cond_ter); 
    ...
    // Rutinas de la libreria LAPACKE
    INFO = LAPACKE_dgesv(LAPACK_ROW_MAJOR, N, NRHS, A, LDA, IPIV, b, LDB);

    print_info_lapacke_solver(INFO);

    // ****************************************************************************
    // * Escritura de results

    // Vaciar resultado obtenido del solver en temper
    get_temper_vector(temper, b, start_temp, end_temp);
