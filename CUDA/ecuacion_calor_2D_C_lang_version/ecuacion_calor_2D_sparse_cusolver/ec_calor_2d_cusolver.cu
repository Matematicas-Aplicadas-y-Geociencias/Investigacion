...
int main(int argc, char const *argv[])
{
    ...
    // ****** VARIABLE DECLARATION: CUSPARSE AND CUSOLVER AND CUDA ******
    double *devVector_b = NULL, *devResults = NULL, *devCsrValues = NULL;
    int *devCsrColumnIndex = NULL, *devCsrRowPointer = NULL;
    ...
    */
    ...
    // DEVICE ARRAYS
    print_memory_info("Antes de asignar memoria en device");
    devVector_b = allocateMemoryVectorDeviceDouble(columns_number);
    devResults = allocateMemoryVectorDeviceDouble(columns_number);
    devCsrValues = allocateMemoryVectorDeviceDouble(nonzero_elements);
    devCsrColumnIndex = allocateMemoryVectorDeviceInteger(nonzero_elements);
    devCsrRowPointer = allocateMemoryVectorDeviceInteger(csr_row_pointer_size);
    print_memory_info("Después de asignar memoria en device");
    ...
    print_vector_double(vector_b, columns_number);
    ...
    transfer_vector_host_to_device_double(vector_b, devVector_b, columns_number);
    transfer_vector_host_to_device_double(results, devResults, columns_number);
    transfer_vector_host_to_device_double(csr_values, devCsrValues, nonzero_elements);
    transfer_vector_host_to_device_integer(csr_column_index, devCsrColumnIndex, nonzero_elements);
    transfer_vector_host_to_device_integer(csr_row_pointer, devCsrRowPointer, csr_row_pointer_size);

    // cusolver_status = cusolverSpDcsrlsvluHost(handle, rows_number, nonzero_elements, descrA, csr_values, csr_row_pointer, csr_column_index, vector_b, TOLERANCE, REORDER, results, &SINGULARITY);
    // cusolver_status = cusolverSpDcsrlsvqr(handle, rows_number, nonzero_elements, descrA, devCsrValues, devCsrRowPointer, devCsrColumnIndex, devVector_b, TOLERANCE, REORDER, devResults, &SINGULARITY);
    cusolver_status = cusolverSpDcsrlsvchol(handle, rows_number, nonzero_elements, descrA, devCsrValues, devCsrRowPointer, devCsrColumnIndex, devVector_b, TOLERANCE, REORDER, devResults, &SINGULARITY);

    cudaDeviceSynchronize();

    // PARALLEL DATA REGION IS CLOSED
    transfer_vector_device_to_host_double(devResults, results, columns_number);

    // ************************************************************************************
    // print_info_cusolver("QR Solver", cusolver_status);
    print_info_cusolver("Cholesky Solver", cusolver_status);
    // print_info_cusolver("LU", cusolver_status);
    ...
    cudaFree(devCsrRowPointer);
    cudaFree(devCsrColumnIndex);
    cudaFree(devCsrValues);
    cudaFree(devResults);
    cudaFree(devVector_b);
