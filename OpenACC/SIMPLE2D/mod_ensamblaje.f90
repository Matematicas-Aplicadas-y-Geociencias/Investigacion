...
contains
  ...
  subroutine ensambla_energia(deltaxpo,deltaypo,&
    ...
    bucle_direccionx: do ii = 2, mi
       !------------------------       
       ! Condiciones de frontera
       if (ii>=placa_mino .and. ii<=placa_maxo) then
          BC_o(1,ii) = 1.0_DBL
          BN_o(1,ii) = 0.0_DBL
          Ry_o(1,ii) = 1.0_DBL
       else
       ...
       !-----------------
       if (ii>=placa_mino .and. ii<=placa_maxo) then
          BC_o(nj+1,ii) = 1.0_DBL
          BN_o(nj+1,ii) = 0.0_DBL
          Ry_o(nj+1,ii) = 1.0_DBL
       else
          BC_o(nj+1,ii)  = 1.0_DBL
          BS_o(nj+1,ii)  =-1.0_DBL
  ...
  subroutine ensambla_corr_pres(deltaxpo,deltaypo,&
    ...
    real(kind=DBL), dimension(mi,nj+1),   intent(in)  :: u_o
    real(kind=DBL), dimension(mi+1,nj),   intent(in)  :: v_o
    real(kind=DBL), dimension(mi+1,nj+1), intent(out) :: corr_pres_o
  ...
  subroutine ensambla_velv(deltaxvo,deltayvo,deltaxuo,&
    ...
    bucle_direccionx: do ii = 2, mi
       ...
       BN_o(1,ii)   = 0.0_DBL
       Ry_o(1,ii)   = 0.0_DBL
       av_o(ii,1)   = 1.e40_DBL !ACj(1)
       BC_o(nj,ii)  = 1._DBL
       BS_o(nj,ii)  = 0.0_DBL
       Ry_o(nj,ii)  = 0.0_DBL
       av_o(ii,nj)  = 1.e40_DBL !ACj(nj+1)       
    end do bucle_direccionx
