...
double precision, dimension(mi,nj) :: resultx,tempx
...
alpha    = 0.5d0 ! par\'ametro de relajaci\'on
 ...
 !
 !$omp target data map( temper(1:mi,1:nj) ) &
 !$omp & map(to: deltax,deltay,temp_ant(1:mi,1:nj),cond_ter,temp_ini,&
 !$omp & temp_fin,flux_aba,flux_arr,alpha,resultx(1:mi,1:nj),resulty(1:nj,1:mi) )&
 !$omp & map(alloc: AI(1:mi,1:nj),AC(1:mi,1:nj),AD(1:mi,1:nj),tempx(1:mi,1:nj),&
 !$omp & BI(1:nj,1:mi),BC(1:nj,1:mi),BD(1:nj,1:mi),tempy(1:nj,1:mi) )
 !
