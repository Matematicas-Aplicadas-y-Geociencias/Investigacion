...
! !$OMP SECTION
!$OMP PARALLEL DO DEFAULT(NONE) PRIVATE(AW,APi,AE,Rxi,uw,ue,vs,vn,wb,wt,a_w,a_e,a_s,a_n,a_b,a_top,delta_x,delta_y)&
!$OMP& PRIVATE(delta_z,alpha,beta,kappa,sigma) SHARED(u_o,v_o,w_o,b_o,au_o,av_o,aw_o,corr_preso,d_xuo,d_yvo,d_zwo,rel_po)
DO k = 2, lk
  DO j = 2, nj
    !***********************
    ...
    DO i = 2, mi
      ...
      Rxi(i)    = alpha*corr_preso(i,j+1,k)+beta*corr_preso(i,j-1,k)+kappa*corr_preso(i,j,k+1)+&
           &sigma*corr_preso(i,j,k-1)+b_o(i,j,k) + APi(i)*(1._DBL-rel_po)*corr_preso(i,j,k)
      !************************************************
    END DO
    !**************************
...
! !$OMP SECTION
!$OMP PARALLEL DO DEFAULT(NONE) PRIVATE(AS,APj,AN,Ryj,uw,ue,vs,vn,wb,wt,a_w,a_e,a_s,a_n,a_b,a_top,delta_x,delta_y,delta_z)&
!$OMP& PRIVATE(alpha,beta,kappa,sigma) SHARED(u_o,v_o,w_o,b_o,au_o,av_o,aw_o,corr_preso,d_xuo,d_yvo,d_zwo,rel_po)
DO k = 2, lk
  DO i = 2, mi
    !***********************
    !Condiciones de frontera
    ...
    DO j = 2, nj
      ...
      b_o(i,j,k)= (uw-ue)*delta_y*delta_z+(vs-vn)*delta_x*delta_z+(wb-wt)*delta_x*delta_y
      Ryj(j)    = alpha*corr_preso(i+1,j,k)+beta*corr_preso(i-1,j,k)+kappa*corr_preso(i,j,k+1)+&
           &sigma*corr_preso(i,j,k-1)+b_o(i,j,k)+APj(j)*(1._DBL-rel_po)*corr_preso(i,j,k)
      !**********************************************
    END DO
    !**************************
...
! !$OMP SECTION
!$OMP PARALLEL DO DEFAULT(NONE) PRIVATE(AB,APk,AT,Rzk,uw,ue,vs,vn,wb,wt,a_w,a_e,a_s,a_n,a_b,a_top,delta_x,delta_y,delta_z)&
!$OMP& PRIVATE(alpha,beta,kappa,sigma) SHARED(u_o,v_o,w_o,b_o,au_o,av_o,aw_o,corr_preso,d_xuo,d_yvo,d_zwo,rel_po)
DO j = 2, nj
  DO i = 2, mi
    !***********************
    ...
    DO k = 2, lk
      ...
      b_o(i,j,k)= (uw-ue)*delta_y*delta_z+(vs-vn)*delta_x*delta_z+(wb-wt)*delta_x*delta_y
      Rzk(k)    = alpha*corr_preso(i+1,j,k)+beta*corr_preso(i-1,j,k)+kappa*corr_preso(i,j+1,k)+&
           &sigma*corr_preso(i,j-1,k)+b_o(i,j,k)+APk(k)*(1._DBL-rel_po)*corr_preso(i,j,k)
      ! **********************************************
    END DO
    ! **************************
...
! !$OMP END PARALLEL
IF(MAXVAL(DABS(fcorr_pres-corr_preso)) < conv_po .or. info > 500)THEN
  WRITE(401,*) 'SALGO: ', MAXVAL(DABS(corr_preso-fcorr_pres)), conv_po, info
  GOTO 3
ELSE
!   WRITE(*,*) info, MAXVAL(DABS(corr_preso-fcorr_pres))
  info = info + 1
  GOTO 2
ENDIF
3 CONTINUE
