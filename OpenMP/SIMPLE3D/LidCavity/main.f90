...
IMPLICIT NONE
INCLUDE 'omp_lib.h'
INTEGER :: i,j,k,kl,l,itera_total,itera,itera_inicial,i_o,i_1,paq_itera
...
REAL(kind=DBL), DIMENSION(mi+1,nj+1,lk+1) :: entropia_calorr,entropia_viscosa,entropia,uf,vf,wf,b_o
...
REAL(kind=DBL)   :: ao
REAL(kind=DBL)   :: tiempo,tiempo_inicial,dt,Pe,Re,Ra,Pr
REAL(kind=DBL)   :: a_ent,lambda_ent
CHARACTER(len=1) :: dec,un,de,ce,m
CHARACTER(len=3) :: mic,njc,lkc,Rac
CHARACTER(len=5) :: sample
CHARACTER(len=46):: entrada_u,entrada_v,entrada_w,entrada_tp,archivo
...
v     = cero! v_ant = cero
w     = cero! w_ant = cero
temp  = cero! temp_ant = cero
corr_pres = cero
au    = 1._DBL
av    = 1._DBL
aw    = 1._DBL
pres(i,j,k)  = cero !1._DBL - z(:)
b_o   = cero
itera = 0
entropia = cero
!************************************************
...
ALGORITMO_SIMPLE: DO       !inicio del algoritmo SIMPLE
    ...
    END DO
    WRITE(401,*) 'velocidad ,itera,', MAXVAL(DABS(du)), MAXVAL(DABS(dv)), MAXVAL(DABS(dw))
    !****************************************
    ...
    DO
      CALL temperatura(x,y,z,fexu,feyv,fezw,d_xu,d_yv,d_zw,u,v,w,temp,temp_ant,gamma_t,dt,dtemp,i_o,i_1,rel_tem)
      !************************************
      !Criterio de convergencia temperatura
      IF(MAXVAL(DABS(dtemp))<conv_t)EXIT
    END DO
    WRITE(401,*) 'temp', MAXVAL(DABS(dtemp))
    !*******************************************
    ...
    WRITE(401,*) 'tiempo ',itera,res_fluido_u,MAXVAL(DABS(Resu)),MAXVAL(DABS(b_o)), conv_paso
    !*************************************************
  END DO ALGORITMO_SIMPLE  !final del algoritmo SIMPLE
  itera = itera + 1
  WRITE(401,*) 'tiempa ',itera,res_fluido_u,MAXVAL(DABS(Resu)),MAXVAL(DABS(b_o))
...
OPEN(unit=1,file='n'//njc//'m'//mic//'k'//lkc//'R'//Rac//'/out_n'//njc//'m'//mic//'k'//lkc//'_R'//Rac//'u.txt')
...
OPEN(unit=2,file='n'//njc//'m'//mic//'k'//lkc//'R'//Rac//'/out_n'//njc//'m'//mic//'k'//lkc//'_R'//Rac//'v.txt')
...
OPEN(unit=3,file='n'//njc//'m'//mic//'k'//lkc//'R'//Rac//'/out_n'//njc//'m'//mic//'k'//lkc//'_R'//Rac//'w.txt')
...
OPEN(unit=4,file='n'//njc//'m'//mic//'k'//lkc//'R'//Rac//'/out_n'//njc//'m'//mic//'k'//lkc//'_R'//Rac//'p.txt')
...
CLOSE(unit=4)
!************************************
!*** Formato de escritura CSV ***
! OPEN(unit=1,file='n'//njc//'m'//mic//'k'//lkc//'R'//Rac//'/t'//m//ce//de//un//dec//'.txt')
! WRITE(1,*) 'x',' y',' z',' Temperature',' U',' V',' W',' Pressure',' Entropy',' Residual'
! DO k = 1, lk+1
!   DO j = 1, nj+1
!     DO i = 1, mi+1
...
archivo = 'n'//njc//'m'//mic//'k'//lkc//'R'//Rac//'/t_'//sample//'.vtk'
CALL postprocess_vtk(x,y,z,uf,vf,wf,pres,temp,b_o,archivo)
