SUBROUTINE temperatura(xo,yo,zo,fexu,feyv,fezw,d_xuo,d_yvo,d_zwo,u_o,v_o,w_o,temp_o,&
     temp_anto,gamma_to,dt_o,dtemp_o,i_oo,i_1o,rel_to)
USE constantes
! USE mkl95_LAPACK
IMPLICIT NONE
INTEGER :: i,j,k,info
...
!*******************
!TDMA en direccion x
!$OMP PARALLEL DO DEFAULT(NONE) PRIVATE(AW,APi,AE,Rxi,uw,ue,vs,vn,wb,wt,dw,de,ds,dn,db,dtop,gamma_w,gamma_e,gamma_s)&
!$OMP& PRIVATE(gamma_n,gamma_b,gamma_top,delta_x,delta_y,delta_z,alpha,beta,kappa,sigma)&
!$OMP& SHARED(u_o,v_o,w_o,temp_o,temp_anto,d_xuo,d_yvo,d_zwo,xo,yo,zo,fexu,feyv,fezw,dt_o,gamma_to,rel_to,i_oo,i_1o)
DO k = 2, lk
  DO j = 2, nj
    !*************************
    ...
    DO i = 2, mi
      ...
      Rxi(i)  = alpha*temp_o(i,j+1,k)+beta*temp_o(i,j-1,k)+kappa*temp_o(i,j,k+1) + &
           &sigma*temp_o(i,j,k-1)+delta_x*delta_y*delta_z*temp_anto(i,j,k)/dt_o+&
...
DO k = 2, lk
  DO i = 2, mi
    ...
    DO j = 2, nj
      ...
      Ryj(j)  = alpha*temp_o(i+1,j,k)+beta*temp_o(i-1,j,k)+kappa*temp_o(i,j,k+1)+sigma*temp_o(i,j,k-1)+&
           &delta_x*delta_y*delta_z*temp_anto(i,j,k)/dt_o+&
...
!********************
!TDMA en direccion  z
!$OMP PARALLEL DO DEFAULT(NONE) PRIVATE(AB,APk,AT,Rzk,uw,ue,vs,vn,wb,wt,dw,de,ds,dn,db,dtop,gamma_w,gamma_e,gamma_s,gamma_n)&
!$OMP& PRIVATE(gamma_b,gamma_top,delta_x,delta_y,delta_z,alpha,beta,kappa,sigma)&
!$OMP& SHARED(u_o,v_o,w_o,temp_o,temp_anto,gamma_to,d_xuo,d_yvo,d_zwo,xo,yo,zo,fexu,feyv,fezw,dt_o,i_oo,i_1o,rel_to)
DO j = 2, nj
  DO i = 2, mi
    !*************************
    ...
    DO k = 2, lk
      ...
      Rzk(k)  = alpha*temp_o(i+1,j,k)+beta*temp_o(i-1,j,k)+kappa*temp_o(i,j+1,k)+sigma*temp_o(i,j-1,k)+&
           &delta_x*delta_y*delta_z*temp_anto(i,j,k)/dt_o+&
