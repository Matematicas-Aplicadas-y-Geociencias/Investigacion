...
DO k = 2, lk
  DO j = 2, nj
    ...
    DO i = 2, mi-1
      ...
      Rxi(i)  = alpha*u_o(i,j+1,k)+beta*u_o(i,j-1,k)+kappa*u_o(i,j,k+1)+sigma*u_o(i,j,k-1)+&
           &delta_x*delta_y*delta_z*u_anto(i,j,k)/dt_o+&
...
!********************
!TDMA en direccion  y
!$OMP PARALLEL DO DEFAULT(NONE) PRIVATE(AS,APj,AN,Ryj,uw,ue,vs,vn,wb,wt,dw,de,ds,dn,db,dtop,gamma_w,gamma_e)&
!$OMP& PRIVATE(gamma_s,gamma_n,gamma_b,gamma_top,delta_x,delta_y,delta_z,alpha,beta,kappa,sigma)&
!$OMP& SHARED(u_o,fu,u_anto,pres_o,gamma_uo,au_o,v_o,w_o,d_xuo,d2_xuo,yo,zo,fey,fez,d_yvo,d_zwo,rel_vo,dt_o)
DO k = 2, lk
  DO i = 2, mi-1
    !*************************
...
!       temp_int = (temp_o(i,j,k)+temp_o(i+1,j,k))/2._DBL
      ...
      Ryj(j)  = alpha*u_o(i+1,j,k)+beta*u_o(i-1,j,k)+kappa*u_o(i,j,k+1)+sigma*u_o(i,j,k-1)+&
           delta_x*delta_y*delta_z*u_anto(i,j,k)/dt_o+&
...
! ********************
!TDMA en direccion  z
!$OMP PARALLEL DO DEFAULT(NONE) PRIVATE(AB,APk,AT,Rzk,uw,ue,vs,vn,wb,wt,dw,de,ds,dn,db,dtop,gamma_w)&
!$OMP& PRIVATE (gamma_e,gamma_s,gamma_n,gamma_b,gamma_top,delta_x,delta_y,delta_z,alpha,beta,kappa,sigma)&
!$OMP& SHARED(u_o,fu,u_anto,pres_o,gamma_uo,au_o,v_o,w_o,d_xuo,d2_xuo,yo,zo,fey,fez,d_yvo,d_zwo,rel_vo,dt_o)
DO j = 2, nj
  DO i = 2, mi-1
    !*************************
    ...
    DO k = 2, lk
      ...
      Rzk(k)  = alpha*u_o(i+1,j,k)+beta*u_o(i-1,j,k)+kappa*u_o(i,j+1,k)+sigma*u_o(i,j-1,k)+&
           delta_x*delta_y*delta_z*u_anto(i,j,k)/dt_o+&
    ...
    APk(lk+1)      = 1._DBL
    AB(lk)         = cero
    Rzk(lk+1)      = cero !1._DBL
    au_o(i,j,lk+1) = APk(lk+1)
    !***************************
