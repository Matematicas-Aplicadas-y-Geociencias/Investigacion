...
!********************
!TDMA en direccion x
!$OMP PARALLEL DO DEFAULT(NONE) PRIVATE(AW,APi,AE,Rxi,uw,ue,vs,vn,wb,wt,dw,de,ds,dn,db,dtop,gamma_w,gamma_e)&
!$OMP& PRIVATE(gamma_s,gamma_n,gamma_b,gamma_top,delta_x,delta_y,delta_z,alpha,beta,kappa,sigma,temp_int)&
...
DO k = 2, lk-1
  DO j = 2, nj
    ...
    DO i = 2, mi
      ...
      Rxi(i)  = alpha*w_o(i,j+1,k)+beta*w_o(i,j-1,k)+kappa*w_o(i,j,k+1)+sigma*w_o(i,j,k-1)+&
           &delta_x*delta_y*delta_z*w_anto(i,j,k)/dt_o+(pres_o(i,j,k)-pres_o(i,j,k+1))*delta_x*delta_y+&
           &APi(i)*(1._DBL-rel_vo)*w_o(i,j,k)+Ri_o(i,j,k)*temp_int*delta_x*delta_y*delta_z
...
!********************
!TDMA en direccion  y
!$OMP PARALLEL DO DEFAULT(NONE) PRIVATE(AS,APj,AN,Ryj,uw,ue,vs,vn,wb,wt,dw,de,ds,dn,db,dtop,gamma_w,gamma_e,gamma_s)&
!$OMP& PRIVATE(gamma_n,gamma_b,gamma_top,delta_x,delta_y,delta_z,alpha,beta,kappa,sigma,temp_int)&
!$OMP& SHARED(w_o,w_anto,pres_o,temp_o,gamma_wo,aw_o,u_o,v_o,d_zwo,d2_zwo,xo,yo,fex,fey,d_xuo,d_yvo,Ri_o,rel_vo,dt_o)
DO k = 2, lk-1
  DO i = 2, mi
    !*************************
    ...
    DO j = 2, nj
      ...
      Ryj(j)  = alpha*w_o(i+1,j,k)+beta*w_o(i-1,j,k)+kappa*w_o(i,j,k+1)+sigma*w_o(i,j,k-1)+&
           &delta_x*delta_y*delta_z*w_anto(i,j,k)/dt_o+(pres_o(i,j,k)-pres_o(i,j,k+1))*delta_x*delta_y+&
           &APj(j)*(1._DBL-rel_vo)*w_o(i,j,k)+Ri_o(i,j,k)*temp_int*delta_x*delta_y*delta_z
...
!********************
!TDMA en direccion  z
!$OMP PARALLEL DO DEFAULT(NONE) PRIVATE(AB,APk,AT,Rzk,uw,ue,vs,vn,wb,wt,dw,de,ds,dn,db,dtop,gamma_w,gamma_e,gamma_s)&
!$OMP& PRIVATE(gamma_n,gamma_b,gamma_top,delta_x,delta_y,delta_z,alpha,beta,kappa,sigma,temp_int)&
!$OMP& SHARED(w_o,w_anto,pres_o,temp_o,gamma_wo,aw_o,u_o,v_o,d_zwo,d2_zwo,xo,yo,fex,fey,d_xuo,d_yvo,Ri_o,rel_vo,dt_o)
DO j = 2, nj
  DO i = 2, mi
    ...
    DO k = 2, lk-1
      ...
      Rzk(k)  = alpha*w_o(i+1,j,k)+beta*w_o(i-1,j,k)+kappa*w_o(i,j+1,k)+sigma*w_o(i,j-1,k)+&
           &delta_x*delta_y*delta_z*w_anto(i,j,k)/dt_o+&
           &(pres_o(i,j,k)-pres_o(i,j,k+1))*delta_x*delta_y+&
           &APk(k)*(1._DBL-rel_vo)*w_o(i,j,k)+Ri_o(i,j,k)*temp_int*delta_x*delta_y*delta_z
