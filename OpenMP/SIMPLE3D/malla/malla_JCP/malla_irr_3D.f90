...
ky=5._DBL
my=1.0_DBL !empaquetamiento en las paredes,menor coeficiente,mayor empaquetamiento
do j=nsolid,nj-nsolid+1
...
do k=1,lk+1
  do j=1,nj+1
    do i=1,mi+1
      write(4,25) y_eta(j),z_eta(k);25 format (2E23.15)
    end do
  end do
end do
close(unit=4)
...
! VTK
  ...
  do k = 1, lk+1
     do j = 1, nj+1
        do i = 1, mi+1
           write(78,100) 0.0, y_eta(j), z_eta(k)
